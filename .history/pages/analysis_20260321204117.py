...
import ta
from utils.features import generate_technical_features, detect_anomalies
...
def plot_interactive_chart(df, ticker_name, supports, resistances):
    """Creates a Plotly chart with candlesticks, volume, S/R levels, and Anomalies."""
    ...
    if not anomalies.empty:
        fig.add_trace(go.Scatter(x=anomalies['Date'], y=anomalies['Close'],
                                 mode='markers',
                                 marker=dict(symbol='star-triangle-down', size=12, color='purple', line=dict(width=2, color='black')),
                                 name='Anomaly Detected',
                                 text="Unusual Volume/Price Action",
                                 hoverinfo="text+x+y"), row=1, col=1)
...
if not df.empty:
    ...
    # UI Layout: Chart
    st.plotly_chart(plot_interactive_chart(df, selected_stock_name, supports, resistances), width="stretch")
